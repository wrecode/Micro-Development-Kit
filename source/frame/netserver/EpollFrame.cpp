...
void EpollFrame::DataMonitor()
{
	...
	while ( !m_stop )
	{
		...
		for ( i = 0; i < nCount; i++ )
		{
			...
			ioList.insert(map<int64,int>::value_type(static_cast<uint64_t>(events[i].data.u64), 1) );//增加可io的对象
		}
...
void EpollFrame::SendAbleMonitor()
{
	...
	while ( !m_stop )
	{
		...
		for ( i = 0; i < nCount; i++ )
		{
			...
			it = ioList.find(events[i].data.u64);
			if ( it != ioList.end() ) continue;
			ioList.insert(map<int64,int>::value_type(static_cast<uint64_t>(events[i].data.u64), 2) );//增加可io的对象
		}
